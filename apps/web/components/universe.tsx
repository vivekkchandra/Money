...
export function UniverseRecords({ page, now }: { page: UniversePage; now: number }) {
  if (page.coverage === "previously_researched_only") return <>
    <p className="muted">Historical eligibility checks from previous research requests—not a current or complete broker directory. A saved record does not establish current broker membership or research eligibility.</p>
  ...
  return <>
    <p className="muted">Reviewed manifest subset: {page.total} mandate-eligible instruments at the last evaluation. This is not the complete Trading 212 accessible universe. Current verified GBX individual stocks only; research admission rechecks all gates.</p>
    ...
    {current.length ? current.map(item => <details className="evidence-item" key={item.instrument_id}><summary>{item.company} · {item.ticker}<span>{item.currency} · Research eligibility verified</span></summary><DataRecord data={{ instrument_type: item.instrument_type, exchange: item.exchange, raw_quote_currency: item.currency, verified_at: item.verified_at, verified_until: item.verified_until, source: item.source, provider: item.provider, source_id: item.source_id, metadata_hash: item.metadata_hash, eligibility_proof_hash: item.eligibility_proof_hash, ethical_proof_hash: item.ethical_proof_hash }} /><Link className="text-button" href={`/jobs?ticker=${encodeURIComponent(item.ticker)}`}>Review company for research →</Link></details>) : <EmptyState title="No current verified instruments on this page" detail="Money fails closed when broker membership or ethical verification is missing or stale. No public listing is substituted for verified availability." icon="search" />}
  </>;
}
